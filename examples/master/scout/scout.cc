...
*/
    ...
    for (int64_t i = 0; i < LOOP_NUMBER; ++i)
    {
        ...
        if ((i % 100) == 0)
        {
            ...
            //for (int j = 0; j<MOTORS; ++j)
            //{
            //    printf("%d\n", input_pdo[j]->actual_position);
            //}


            printf("[%d] %x (%x) position %d to %d\n",
                i, state_machine[0].getControlWord(), input_pdo[0]->status_word,

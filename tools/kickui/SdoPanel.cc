...
namespace kickcat::kickui
{
    namespace
    {
        // Writer type picker = the library's basic data types (sizes/signedness from
        // CoE::basicDataTypes(), names from CoE::toString -- no per-type knowledge
        // duplicated here) plus a UI-only raw-hex fallback (DataType::UNKNOWN, last).
        std::vector<CoE::DataTypeInfo> const& writerTypes()
        {
            static std::vector<CoE::DataTypeInfo> const list = []
            {
                std::vector<CoE::DataTypeInfo> v(CoE::BASIC_DATA_TYPES.begin(), CoE::BASIC_DATA_TYPES.end());
                v.push_back({CoE::DataType::UNKNOWN, 0, false, false, false});  // RAW (hex)
                return v;
            }();
            return list;
        }

        char const* typeLabel(CoE::DataTypeInfo const& t)
        {
            if (t.type == CoE::DataType::UNKNOWN) { return "RAW (hex)"; }
            return CoE::toString(t.type);
        }

        // The writerTypes() index for a discovered CoE DataType; the raw-hex entry
        // (last) if the type is not one we offer.
        int typeIndexFor(CoE::DataType dt)
        {
            auto const& types = writerTypes();
            for (int i = 0; i < static_cast<int>(types.size()); ++i)
            {
                if (types[i].type == dt) { return i; }
            }
        ...
        std::string formatValue(CoE::DataTypeInfo const& t, std::vector<uint8_t> const& data)
        {
            if (data.empty())
            {
                return "(empty)";
            ...
            if (t.size == 0)   // raw-hex (or any variable-length type)
            {
                return hexDump(data);
            }
        ...
        std::vector<uint8_t> bytesFromValue(CoE::DataTypeInfo const& t, char const* text, bool& ok)
        {
            ok = true;
            if (t.is_string)
            {
            ...
            if (t.is_real)
            {
                ...
                ok = (end != text);   // at least one char consumed
            ...
            ok = (end != text) and (errno != ERANGE);   // parsed something, not overflowed
            std::vector<uint8_t> out(t.size);
    ...
    void SdoPanel::render(BusSession&, Device& device)
    {
        ...
        auto const& types = writerTypes();
        CoE::DataTypeInfo const& type = types[type_];
        ...
        if (ImGui::BeginCombo("Type", typeLabel(type)))
        {
            for (int i = 0; i < static_cast<int>(types.size()); ++i)
            {
                bool selected = (i == type_);
                if (ImGui::Selectable(typeLabel(types[i]), selected))
                {
                    type_ = i;
                }
            }
        ...
        {
            bool ok = false;
            std::vector<uint8_t> bytes = bytesFromValue(type, value_buf_, ok);
            if (ok)
            {
                last_ = device.writeSDO(parseIndex(index_buf_),
                                         static_cast<uint8_t>(subindex_), access_, bytes);
                write_msg_.clear();
            }
            else
            {
                write_msg_ = "invalid numeric value";
            }
        }
        ImGui::EndDisabled();
        ImGui::SameLine();
        ...
        if (not write_msg_.empty())
        {
            ImGui::TextDisabled("%s", write_msg_.c_str());
        }
        ...
        if (last_)
        {
            ...
            else if (last_->ok)
            {
                ...
                {
                    ImGui::Text("value: %s", formatValue(writerTypes()[last_type_], last_->data).c_str());
    ...
    void SdoPanel::renderOdTable(std::vector<OdObject> const& objects)
    {
        ...
        auto valueCell = [&](CoE::DataType data_type, std::vector<uint8_t> const& value,
                             std::string const& err, uint16_t access)
        {
            if (not value.empty())
            {
                ImGui::TextUnformatted(formatValue(writerTypes()[typeIndexFor(data_type)], value).c_str());
            }
        ...
        for (auto const& obj : objects)
        {
            ...
            bool complex = (obj.object_code == CoE::ObjectCode::ARRAY)
            ...
            ImGui::TableNextColumn();
            if (complex) { ImGui::TextUnformatted(CoE::toString(obj.object_code)); }
            else         { ImGui::TextUnformatted(CoE::toString(obj.data_type)); }
            ImGui::TableNextColumn();
            ...
            for (auto const& se : obj.entries)
            {
                ...
                ImGui::TableNextColumn();
                ImGui::TextUnformatted(CoE::toString(se.data_type));
                ImGui::TableNextColumn();

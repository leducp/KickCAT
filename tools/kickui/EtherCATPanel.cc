...
namespace kickcat::kickui
{
    ...
    void EtherCATPanel::renderMappingEditor(Device& device)
    {
        ...
        {
            if (seedFromReadback(device, false)) 
			{ 
				readback_msg_.clear(); 
			}
            else 
			{ 
				readback_msg_ = "no read-back yet -- \"Read PDO mapping\" or reach OP first"; 
			}
        }
        if (not readback_msg_.empty())
        {
            ImGui::TextDisabled("%s", readback_msg_.c_str());
        }

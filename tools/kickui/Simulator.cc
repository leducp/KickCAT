...
namespace kickcat::kickui
{
    ...
    bool SimulatorProcess::launch(SimScene const& scene, std::string& error)
    {
        ...
        catch (std::exception const& e)
        {
            ...
            releaseControl();               // drop the (possibly half-created) segment
            ::unlink(topo_path_.c_str());   // topology file was written just above
            topo_path_.clear();
            return false;
        }
    ...
    void SimulatorProcess::stop()
    {
        ...
        {
            ::unlink(topo_path_.c_str());
            topo_path_.clear();   // match reap()'s postcondition
        }
    }
    ...
    bool SimulatorProcess::nextEvent(sim::ControlEvent& out)
    {
        if (not control_) { return false; }
        return control_->nextEvent(out);
    }

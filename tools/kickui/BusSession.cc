...
namespace kickcat::kickui
{
    ...
    namespace
    {
                                ...
                                std::vector<PdoEntry> const& entries, uint16_t sm_map)
        {
            ...
            {
                words.push_back((static_cast<uint32_t>(e.index) << 16)
                              | (static_cast<uint32_t>(e.sub)   << 8)
                              |  static_cast<uint32_t>(e.bits));
            }
    ...
    void BusSession::rtLoop()
    {
        ...
        try
        {
            ...
            for (auto& r : rts) { if (r.drive) { r.drive->attach(); } }
        ...
        try
        {
            ...
            auto readAssign = [&](uint16_t sm_assign, std::vector<PdoEntry>& out)
            {
                ...
                for (uint32_t i = 1; i <= pdo_count; ++i)
                {
                    ...
                    {
                        uint32_t e = readU(slave, pdo, static_cast<uint8_t>(j), 4);
                        PdoEntry pe;
                        pe.pdo   = pdo;
                        pe.index = static_cast<uint16_t>(e >> 16);
                        pe.sub   = static_cast<uint8_t>(e >> 8);
                        pe.bits  = static_cast<uint8_t>(e & 0xFF);
                        out.push_back(pe);
                    }
                }
            };
        ...
        try
        {
            ...
            for (; (i < n) and (consec_fail < FAIL_LIMIT); ++i)
            {
                ...
                if (driveMessage(od, cyclic) and (bsize >= sizeof(info::ObjectDescription)))
                {
                    ...
                    obj.data_type    = static_cast<uint16_t>(d->data_type);
                    obj.max_subindex = d->max_subindex;
                    obj.object_code  = static_cast<uint8_t>(d->object_code);
                ...
                auto readEntry = [&](uint8_t sub, bool& ed_ok, bool fallback) -> OdEntry
                {
                    ...
                    if (ed_ok)
                    {
                        ...
                        e.access    = d->access;
                        e.data_type = static_cast<uint16_t>(d->data_type);
                ...
                bool is_complex = (obj.object_code == static_cast<uint8_t>(CoE::ObjectCode::ARRAY))

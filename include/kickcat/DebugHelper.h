...
namespace kickcat
{
    ...
    template<typename T>
    void sendWriteRegister(Link& link, uint16_t slave_address, uint16_t reg_address, T const& value_write)
    {
        ...
        auto error = [](DatagramState const&)
        {
            THROW_ERROR("Error while trying to write slave register.");
        };

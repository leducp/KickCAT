...
namespace kickcat
{
    ESCEmulator::ESCEmulator(std::string const& eeprom)
        : memory_{}
    {
    ...
    int32_t ESCEmulator::computeInternalMemoryAccess(uint16_t address, void* buffer, uint16_t size, Access access)
    {
    ...
    int32_t ESCEmulator::write(uint16_t address, void const* data, uint16_t size)
    {
        return computeInternalMemoryAccess(address, const_cast<void*>(data), size, Access::PDI_WRITE);
    }

    int32_t ESCEmulator::read(uint16_t address, void* data, uint16_t size)
    {
        return computeInternalMemoryAccess(address, data, size, Access::PDI_READ);
    }


    void ESCEmulator::processDatagram(DatagramHeader* header, uint8_t* data, uint16_t* wkc)
    {
    ...
    uint16_t ESCEmulator::processPDO(std::vector<PDO> const& pdos, bool read, DatagramHeader* header, uint8_t* data)
    {
        int wkc = 0;
    ...
    void ESCEmulator::processLRD(DatagramHeader* header, uint8_t* data, uint16_t* wkc)
    {
        *wkc += processPDO(rx_pdos_, true, header, data);
    }

    void ESCEmulator::processLWR(DatagramHeader* header, uint8_t* data, uint16_t* wkc)
    {
        *wkc += processPDO(tx_pdos_, false, header, data);
    }

    void ESCEmulator::processLRW(DatagramHeader* header, uint8_t* data, uint16_t* wkc)
    {
    ...
    void ESCEmulator::processEcatRequest(DatagramHeader* header, uint8_t* data, uint16_t* wkc)
    {
    ...
    void ESCEmulator::processInternalLogic()
    {
    ...
    void ESCEmulator::processReadCommand(DatagramHeader* header, uint8_t* data, uint16_t* wkc, uint16_t offset)
    {
    ...
    void ESCEmulator::processWriteCommand(DatagramHeader* header, uint8_t* data, uint16_t* wkc, uint16_t offset)
    {
    ...
    void ESCEmulator::processReadWriteCommand(DatagramHeader* header, uint8_t* data, uint16_t* wkc, uint16_t offset)
    {
        uint8_t swap[MAX_ETHERCAT_PAYLOAD_SIZE];
        std::memcpy(swap, data, header->len);
    ...
    void ESCEmulator::configurePDOs()
    {
        tx_pdos_.clear();
        rx_pdos_.clear();
        for (int i = 0; i < 16; ++i)
    ...
    std::tuple<uint8_t*, uint8_t*, uint16_t> ESCEmulator::computeLogicalIntersection(DatagramHeader const* header, uint8_t* data, PDO const& pdo)
    {
    ...
    nanoseconds ESCEmulator::pdiWatchdog()
    {
    ...
    nanoseconds ESCEmulator::pdoWatchdog()
    {
        nanoseconds divider = (memory_.watchdog_divider + 2) * 40ns;
    ...
    void ESCEmulator::checkWatchdog()
    {

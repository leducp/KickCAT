...
namespace kickcat
{
    using namespace CoE::SDO::abortcode;
    char const* CoE::SDO::abortcode::abort_to_str(uint32_t abort_code)
    {
        switch (abort_code)
        {
            case TOGGLE_BIT_NOT_CHANGED:             { return "Toggle bit not changed";                                                                     }
            case TIMEOUT:                            { return "SDO protocol timeout";                                                                       }
            case INVALID_COMMAND:                    { return "Client/Server command specifier not valid or unknown";                                       }
            case OUT_OF_MEMORY:                      { return "Out of memory";                                                                              }
            case UNSUPPORTED_ACCESS:                 { return "Unsupported access to object";                                                               }
            case WRITE_ONLY:                         { return "Attempt to read a write only object ";                                                       }
            case READ_ONLY:                          { return "Attempt to write a read only object";                                                        }
            case CANT_WRITE_SI:                      { return "Subindex cannot be written, SI0 must be 0 for write access";                                 }
            case CA_NOT_SUPPORTED:                   { return "SDO Complete access not supported for objects of variable length";                           }
            case OBJECT_TOO_BIG:                     { return "Object length exceeds mailbox size";                                                         }
            case OBJECT_MAPPED_TO_RXPDO:             { return "Object mapped to RxPDO, SDO Download blocked";                                               }
            case OBJECT_DOES_NOT_EXIST:              { return "The object does not exist in the object dictionnary";                                        }
            case CANT_MAP_OBJECT_TO_PDO:             { return "The object cannot be mapped into the PDO";                                                   }
            case EXCEED_PDO_LENGHT:                  { return "The number and length of the objects to be mapped would exceed the PDO lenght";              }
            ...
            case SERVICE_PARAMETER_LENGHT_NOT_MATCH: { return "Data type does not match, length of service parameter does not match";                       }
            case SERVICE_PARAMETER_LENGHT_TOO_HIGH:  { return "Data type does not match, length of service parameter too high";                             }
            case SERVICE_PARAMETER_LENGHT_TOO_LOW:   { return "Data type does not match, length of service parameter too low";                              }
            case SUBINDEX_DOES_NOT_EXIST:            { return "Subindex does not exist";                                                                    }
            case PARAMETER_RANGE_EXCEEDED:           { return "Value range of parameter exceeded";                                                          }
            case PARAMATER_VALUE_TOO_HIGH:           { return "Value of parameter written too high";                                                        }
            case PARAMATER_VALUE_TOO_LOW:            { return "Value of parameter written too low";                                                         }
            case MODULE_LIST_DOES_NOT_MATCH:         { return "Configured module list does not match detected module list";                                 }
            case MAX_VALUE_SMALLER_THAN_MIN:         { return "Maximum value is less than minimum value";                                                   }
            case GENERAL_ERROR:                      { return "General error";                                                                              }
            case CANT_STORE_DATA:                    { return "Data cannot be transferred or stored to the application";                                    }
            case CANT_STORE_DATA_LOCAL_CONTROL:      { return "Data cannot be transferred or stored to the application because of local control";           }
            case CANT_STORE_DATA_DEVICE_STATE:       { return "Data cannot be transferred or stored to the application because of the present device state";}
            case MISSING_OBJECT_DICTIONNARY:         { return "Object dictionnary dynamic generation fails or no object dictionnary is present";            }
